    ...
    if (screen === "game") {
      ...
      const animate = () => {
        ...
        if (player.isAttacking) {
          newAction = player.lastAttackType;
        } else {
          if (freezeControls) {
            player.velocity.x = 0;
            newAction = "stunned";
        ...
        if (
          player.isAttacking &&
          player.framesCurrent === player.sprites[player.lastAttackType].framesMax - 1
        ) {
          setTimeout(() => {
            if (player.isAttacking) {
    ...
    <div className="game-container">
      ...
      {screen === "game" && (
        <div className={"game-canvas"}>
          ...
          <div className={`game-results ${gameResult === "playerWins" ? "show" : ""}`}>
            <div className="game-over winner">
              <h2>You Win!</h2>
              <button className="play-again" onClick={resetGame}>Play Again</button>
            </div>
          </div>
          <div className={`game-results ${gameResult === "enemyWins" ? "show" : ""}`}>
